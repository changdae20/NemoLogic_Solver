...
std::vector<std::vector<int>> make_candidates(const std::vector<int>& hint, int board_size) {
    ...
    for (size_t i = 0; i < hint.size(); ++i) temp[i] = false; // temp = [(false) * hint.size() , (true) * (board_size - hint.size() + 1)], total size of board_size.
    ...
    do {
        ...
        for (const auto& el : temp) {
            if (!el) {
                for (int i = 0; i < hint.at(hint_index); ++i) candidate.push_back(static_cast<int>(CELL::FILL));
                if (hint_index < hint.size() - 1) candidate.push_back(static_cast<int>(CELL::BLANK));
                ++hint_index;
            }
            else {
...
bool is_done(const std::vector<std::vector<int>>& board) {
    for (const auto& row : board)
        for (const auto& el : row)
            if (el == static_cast<int>(CELL::UNKNOWN)) return false;
    return true;
}
...
bool perform_row(std::vector<std::vector<int>>& board, std::vector<std::vector<int>>& row_candidates, int row) {
    ...
    for (size_t i = 0; i < board[row].size(); ++i) {
        for (auto iter = row_candidates.begin(); iter != row_candidates.end();) {
            if (board[row][i] == static_cast<int> (CELL::UNKNOWN)) ++iter;
...
bool perform_col(std::vector<std::vector<int>>& board, std::vector<std::vector<int>>& col_candidates, int col) {
    ...
    for (size_t i = 0; i < board.size(); ++i) {
        for (auto iter = col_candidates.begin(); iter != col_candidates.end();) {
            if (board[i][col] == static_cast<int> (CELL::UNKNOWN)) ++iter;
...
void print_board(std::vector<std::vector<int>> &board) {
    for (const auto& row : board) {
        for (const auto& el : row) {
            if (el == static_cast<int> (CELL::FILL)) std::cout << "O";
            else if (el == static_cast<int> (CELL::BLANK)) std::cout << " "; 
            else if (el == static_cast<int> (CELL::UNKNOWN)) std::cout << "?";
        }
        std::cout << std::endl;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < 10000; ++i) {
        ...
        if(j<board.size()) row_candidates[j].second.second = row_candidates[j].first.size();
        std::sort(row_candidates.begin(), row_candidates.end(), [](const auto& a, const auto& b){return a.second.second < b.second.second;});
        ...
        if(j<board.size()) col_candidates[j].second.second = col_candidates[j].first.size();
        std::sort(col_candidates.begin(), col_candidates.end(), [](const auto& a, const auto& b){return a.second.second < b.second.second;});
        
        system("cls");
        std::cout << "===== " << i + 1 << "th iteration =====" << std::endl;
        print_board(board);
        if (is_done(board)) break;
    }
    system("cls");
    std::cout << "===== Result =====" << std::endl;
    print_board(board);
    if (is_done(board)) std::cout << "DONE!!" << std::endl;
